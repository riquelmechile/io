...
- [x] 1.9 GREEN `ports/repositories.ts`: `LearningCandidateRepository` port (`appendInitial`|`appendTransition`|`getCurrent`|`listRevisions`) + typed results `appended`|`replayed`|`stale`|`conflict`|`idempotency-collision`; export all new symbols from `business-domain/src/index.ts`.
