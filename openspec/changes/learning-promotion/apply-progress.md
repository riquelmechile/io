...
### Correction pass (R3-001) — bounded 96-line budget, one transaction
Reliability-lens CRITICAL fix: `evaluatePromotion` now binds EVERY explicit observation to the candidate (companyId equality AND subject.skillId/skillVersion equality) BEFORE escalation and threshold logic. Foreign `conflicts`/`catastrophicVetoes`/`rateObservations` are ignored (never escalated, never counted); foreign scalar `confidence`/`sourceAuthority` become unavailable (typed `*-unavailable` remain-candidate when policy requires them, never binding, never `source-authority-not-allowed`). Missing stays unknown, never harmful. Strict TDD: RED **4F/22P** → GREEN **26/26** focused; full gate **1524 passed / 6 skipped**; 0 new warnings (9 pre-existing); new tests: foreign conflict/veto/rate/scalar binding (4 its, same test file). No ports/index/corpus changes.
