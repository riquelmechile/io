...
Rollback: revert the `InMemoryPromotionAuthorityRepository` block + `PromotionAuthorityFakeSnapshot` from fakes.ts/index + drop core test + this section — no consumers (coverage tests, app seam, PG adapter not landed yet).
