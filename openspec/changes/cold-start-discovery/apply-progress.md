...
## Focused Remediation Record (2026-08-13, work unit `no-backfill-runtime-coverage`)

- **Trigger**: verify-report FAIL blocker "UNTESTED: Pre-change accepted Work is not backfilled has no covering runtime test" (sdd-verify owns the report; NOT rewritten here — it stays FAIL until sdd-verify refreshes it).
- **Acquire**: native state `proceed`, max attempts 1, max changed lines **180**, opaque token parent-held (no acquire/settle/reset by executor). Test-only remediation: NO production source/spec/design/proposal change; tasks.md 18/18 untouched (no new task entry required); no commit/stage/push/archive/review; parent owns settle/reverification.
- **Proof (spec business-event "Pre-change accepted Work is not backfilled" / design D8)**: new live-PG case in `cold-start-e2e.integration.test.ts` — GIVEN pre-change persisted state (accepted Work, ZERO `business_event` rows, seeded via `seedAcceptedWork`, NO bypass of the D10 real-path proof which remains in the first describe) → discovery `listCompanyIds()` returns `[]` (company not newly discoverable) → real supervisor one-shot pump → no `work.accepted` synthesized (event log still empty), no activation (0 receipts, 0 LLM plans), historical Work untouched `accepted`@v1.
- **Focused command + result**: `DATABASE_URL='postgresql://io:io_dev@127.0.0.1:5434/io_dev' IO_REQUIRE_PG=1 PATH=/data/node24/bin:$PATH pnpm vitest run --no-file-parallelism packages/app/test/e2e/cold-start-e2e.integration.test.ts` → **Test Files 1 passed (1); Tests 3 passed (3)** = 2 pre-existing + 1 new; exit 0; NO skip (raw: `evidence/remediation-no-backfill-focused.log`). Harness identity verified pre-start (`io-phase4-iso-pg-3cdff5d7`, postgres:18.4, loopback 127.0.0.1:5434, vol `io-phase4-iso-pg-vol-3cdff5d7`); started → run → stopped, preserved; shared `io_pg`/`io_dev` never used; `.pi/` untouched.
- **Rollback boundary**: revert = delete the no-backfill `describe` block (+ `seedAcceptedWork`/`E2E_WORK_ID` import lines) from `cold-start-e2e.integration.test.ts`, delete `evidence/remediation-no-backfill-focused.log`, strip this record + accounting line → clean return to HEAD `80c8dfc`; zero production/spec/design/proposal/task-state changes to revert. Reverting this remediation never removes Phase 1–5 work.

### TDD Cycle Evidence (remediation — coverage RED = verifier's UNTESTED state, not a product failure)

| Task | Test File | Layer | Safety Net | RED | GREEN | TRIANGULATE | REFACTOR |
|------|-----------|-------|------------|-----|-------|-------------|----------|
| no-backfill (normative proof) | `cold-start-e2e.integration.test.ts` (new describe) | E2E (live PG) | ✅ existing file 2/2 passing pre-edit; app e2e dir 10 passed/1 skipped pre-edit | ✅ scenario previously UNTESTED (verify-report blocker 23/24); test written to demonstrate it — passes immediately because behavior is already correct (no production edit, no forced RED) | ✅ `1 passed (1); 3 passed (3)` exit 0, no skip | ✅ companion non-empty case in same file (`listCompanyIds()` → `[E2E_COMPANY]` after an event) proves the empty discovery result is real production logic, not a vacuous assertion | ✅ Biome format converged (no fixes); `biome lint` exit 0; tsc clean |

### Work Unit Evidence (remediation)

| Evidence | Required value |
|----------|----------------|
| Focused test command and exact result | `DATABASE_URL='postgresql://io:io_dev@127.0.0.1:5434/io_dev' IO_REQUIRE_PG=1 PATH=/data/node24/bin:$PATH pnpm vitest run --no-file-parallelism packages/app/test/e2e/cold-start-e2e.integration.test.ts` → **1 passed (1); 3 passed (3)**, exit 0, NO skip (raw: `evidence/remediation-no-backfill-focused.log`); app e2e dir safety net **10 passed/1 skipped (11); 23 passed/3 skipped (26)** |
| Runtime harness command/scenario and exact result | **LIVE PostgreSQL (dedicated isolated)**: container `io-phase4-iso-pg-3cdff5d7` (postgres:18.4) + vol `io-phase4-iso-pg-vol-3cdff5d7`, loopback `127.0.0.1:5434`; identity verified pre-start. Scenario: pre-change accepted Work persisted with NO event → `listCompanyIds()` `[]` → real supervisor one-shot pump → event log still 0 rows (no synthesis), 0 receipts, 0 LLM plans (no activation), Work untouched `accepted`@v1. Started → run → stopped, preserved; shared `io_pg`/`io_dev` never used |
| Rollback boundary | Revert the no-backfill describe block + import lines from `cold-start-e2e.integration.test.ts`, delete `evidence/remediation-no-backfill-focused.log`, strip this remediation record → clean return to HEAD `80c8dfc`; zero production/spec/design/proposal/task-state changes; never removes Phase 1–5 work |

- **Accounting vs HEAD `80c8dfc`**: tracked +81 (`cold-start-e2e.integration.test.ts` +57, `apply-progress.md` +24); untracked new +14 (`evidence/remediation-no-backfill-focused.log`) = **95 authored delta ≤ 180 ✓**. Pre-existing untracked `verify-report.md` (113 lines, sdd-verify artifact, UNTOUCHED) excluded from this authored candidate — parent decides whether it enters candidate accounting (if counted, combined 208 > 180 is a parent decision point, not an executor violation; the 180 cap reads on the remediation candidate); `.pi/` machine-local excluded.
