...
Hexagonal outbound port boundary for evidence (R7) and audit (R16) storage inside the transitional `packages/trust-kernel/`. Defines ASYNC repository port interfaces, a durable `PersistentRecord` type diverging from `InMemoryRecord`, in-memory fake adapters, and backward-compatible ASYNC pipeline wiring. The ports are async (`Promise`-returning) so a real PostgreSQL-backed adapter may be wired downstream in `packages/database/`; `pg` is permitted as a runtime dependency there and integration tests are enabled (`integration: true`). Real PostgreSQL transactions/sessions, canonical extraction, other aggregates, crypto receipts, real approval chains, and R1–R17 durability obligations remain explicitly deferred.
