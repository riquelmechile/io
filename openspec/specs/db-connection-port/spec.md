...
Injectable ASYNC `DbConnection` port plus the PostgreSQL-shaped adapters implementing the kernel `EvidenceRepository<R>` / `AuditRepository<R>` ports, in the Technical-Infrastructure package `packages/database/`. Proves the adapter side of `persistence-port-boundary` WITH live PostgreSQL: the connection is async (`Promise`-returning), SQL is PG-shaped (`$N`), a real `PgDbConnection` over `pg.Pool` enables integration testing under `integration: true`, and an in-memory fake enables unit testing. Transactions/sessions (`DbSession`), migration runner, pool-lifecycle tuning, and R1–R17 durability obligations remain deferred.
